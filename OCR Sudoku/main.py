...
#     cv2.imwrite('greyscale.png',temp)

#     blur=cv2.GaussianBlur(temp, (9,9), 0)
...
#     cv2.imwrite('invThresh.png',~thresh1)

#     #invThresh=cv2.imread('invThresh.png')

#     tres=100
#     ret,tres_img=cv2.threshold(temp,tres,255,cv2.THRESH_BINARY)
#     contours, hierarchy = cv2.findContours(tres_img, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
#     img_contours = np.zeros(img.shape)
#     cv2.drawContours(img_contours, contours, -1, (0,255,0), 3, cv2.LINE_AA)
#     cv2.imwrite('contour.png',img_contours)

#     corners = cv2.goodFeaturesToTrack(temp,4,0.01,2475,useHarrisDetector=True,k=0.04)
...
#     # contoured = cv2.imread('contour.png')
#     # contoured = np.float32(contoured)
#     # contoured=cv2.cvtColor(contoured,cv2.COLOR_RGB2GRAY)

#     # dst = cv2.cornerHarris(contoured,2,3,0.01)
